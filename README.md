...
<h5>● 模型的响应速度与任务的复杂度以及您的硬件能力有关</h5>
<h5>● 在每次更新项目时，请注意是否有数据库更改，如果有请执行sql脚本，避免运行出错</h5>
<h5>● 如果要使用mcp服务请先在mcp-server模块中配置相关配置，尤其是涉及到调用外部API(例如天气查询、IP定位等),请自行配置，现有配置可能无法正常使用</h5>
